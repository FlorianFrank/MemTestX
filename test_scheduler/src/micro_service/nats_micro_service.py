...
from messaging_service_config import Topic, get_topic_str
from micro_service.device_list import DeviceList

from micro_service.error_msg import ErrorMsg, ErrorCode
...
class NATSMessagingService(MessagingServiceBase):
    ...
    async def publish_topic(self, data: dict) -> ErrorMsg:
        ...
        """
        try:
            self._logger.info('Publish to topic {}:{}'.format(data['topic'], data['data']))
            await self._nats_handle.publish(data['topic'], data['data'].encode('utf-8'), reply=self.inbox)
            return ErrorMsg()
