...
from device_types.PowerSupply import PSMode, PSChannel
from devices.SPD1305X import SPD1305X

IP_ADDR = '132.231.14.109'
PORT = 5025
TIME_OUT = 5
MAX_DEVIATION_VOLTS = 0.01
MAX_DEVIATION_CURRENT = 0.001
...
class PowerSupplyWrapper:
    ...
    def initialize(self):
        logger.info(f"Setup SPD1305X with parameters {IP_ADDR}:{PORT} Timeout {TIME_OUT}")
        self._dev_config = TCPDeviceConfig(ip=IP_ADDR, port=PORT, timeout=TIME_OUT)
    ...
    def power_supply_default_settings(self, default_current: float, default_voltage: float) -> bool:
        ...
        for i in range(4):
            if time_out or i == 0:
                ...
                try:
                    ...
                    time.sleep(i + 1)
                if not time_out:
                    if abs(ret_voltage) - MAX_DEVIATION_VOLTS < default_voltage < abs(
                            ret_voltage) + MAX_DEVIATION_VOLTS:
        ...
        for i in range(4):
            ...
            try:
                ...
                time.sleep(i + 1)
            if not time_out:
                if abs(ret_current) - MAX_DEVIATION_CURRENT < default_current < abs(ret_current) + MAX_DEVIATION_CURRENT:
        ...
        if not system_state['output']:
            if system_state['wire_mode'] == PSMode.W2:
                if not system_state['timer']:
                    if system_state['display'] is 'digital_display':
            ...
            for i in range(4):
                if time_out or i == 0:
                    ...
                    try:
                        ...
                        time.sleep(i + 1)
                    if not time_out:
                        if abs(ret_voltage) - MAX_DEVIATION_VOLTS < voltage < abs(ret_voltage) + MAX_DEVIATION_VOLTS:
            ...
            for i in range(4):
                if time_out or i == 0:
                    ...
                    try:
                        ...
                        time.sleep(i + 1)
                    if not time_out:
                        if abs(ret_current) - MAX_DEVIATION_CURRENT < current < abs(ret_current) + MAX_DEVIATION_CURRENT:
