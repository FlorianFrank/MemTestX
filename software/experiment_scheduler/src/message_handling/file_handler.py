...
class MeasureFileHandler:
    """Handles storing and retrieving measurement data from CSV files"""

    def __init__(self, measure_file_path: str, file_name: str):
        self._lock = threading.Lock()
        self._store_data_thread = None
        self._transmit_data_thread_running = False
        self._file_name = file_name
        self._path = measure_file_path
        ...
        logger.info(f"MeasureFileHandler initialized with path: {measure_file_path}, file_name: {file_name}")

    def get_file_name(self) -> str:
        return self._file_name

    def initialize(self) -> bool:
        """Open or create a new measurement file and initialize CSV reader/writer."""
        with self._lock:
            if self._is_open:
            ...
            try:
                ...
                self._csv_writer = csv.writer(self._file_handle, delimiter=MEASURE_FILE_DELIMITER,
                                              quotechar=MEASURE_FILE_QUOTE_CHAR,
                                              quoting=csv.QUOTE_MINIMAL)
                self._csv_reader = csv.reader(self._file_handle, delimiter=MEASURE_FILE_DELIMITER,
                                              quotechar=MEASURE_FILE_QUOTE_CHAR)
                self._is_open = True
    ...
    def write_data(self, data: List[MeasureDataTuple]):
        """Write a list of MeasureDataTuple entries to the CSV file."""
        #with self._lock:
        if self._is_open:
            try:
    ...
    def read_file(self) -> np.ndarray:
        """Read data from the CSV file and return it as a structured NumPy array."""
        with self._lock:
    ...
    def store_data_thread_func(self):
        """Background thread function that continuously writes buffered data to the file."""
    ...
    def start_store_data_thread(self):
        """Start the background data storage thread if not already running."""
    ...
    def stop_store_data_thread(self):
        """Stop the background data storage thread safely."""
    ...
    def add_to_buffer(self, data: List):
        """Add a batch of new measurement entries to the internal buffer."""
        #with self._lock:
        try:
            for d in data:
    ...
    def close_file(self):
        """Close the measurement file and clear all buffered data."""
        with self._lock:
            if self._is_open:
                try:
