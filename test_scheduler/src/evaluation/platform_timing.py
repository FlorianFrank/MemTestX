...
def plot_platform_timing(config_file: str, board_name: str, board_frequency: int):
    ...
    ax.errorbar(cycles, ce_mean, yerr=ce_std, fmt='o', capsize=5, markersize=3, label="CE (Mean ± Std Dev)")
    ax.errorbar(cycles, we_mean, yerr=we_std, fmt='o', capsize=5, markersize=3, label="WE (Mean ± Std Dev)")
    ax.errorbar(cycles, oe_mean, yerr=oe_std, fmt='o', capsize=5, markersize=3, label="OE (Mean ± Std Dev)")
